    ...
    class Solution {
        public int myAtoi(String s) {
            ...
            for (char c : s.toCharArray()) {
                atoi.get(c);
                if (atoi.isEnd()) {
                    return (int) (atoi.getRes() * atoi.assign);
                }
            }
            return (int) (atoi.getRes() * atoi.assign);
        }
        ...
        class Atoi {
            ...
            private String startStep = "start";
            ...
            public void get(char c) {
                startStep = step.get(startStep)[getStep(c)];
                if ("inNumber".equals(startStep)) {
                    res = res * 10 + Integer.parseInt(String.valueOf(c));
                    res = assign == 1 ? Math.min(res,(long) Integer.MAX_VALUE) : Math.min(res,((long) Integer.MIN_VALUE) * -1);
                }
                if ("assign".equals(startStep)) {
                    assign = c == '-' ? -1 : 1;
                }
                if ("end".equals(startStep)) {
                    end = true;
                }
            }

            private int getStep(char c) {
                if (c == ' ') {
                    return 0;
                } else if (c == '+' || c == '-') {
                    return 1;
