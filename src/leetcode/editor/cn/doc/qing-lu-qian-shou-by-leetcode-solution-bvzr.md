...
如果图中形成了一个大小为 *k* 的环：![i\rightarrowj\rightarrowk\rightarrow\ldots\rightarrowl\rightarrowi ](./p__i_rightarrow_j_rightarrow_k_rightarrow_ldots_rightarrow_l_rightarrow_i_.png) ，则我们沿着环的方向，先交换 *i,j* 的位置，再交换 *j,k* 的位置，以此类推。在进行了 *k-1* 次交换后，这 *k* 对情侣就都能够彼此牵手了。
...
- 时间复杂度：![O(N\logN) ](./p__O_N_log_N__.png) ，其中 *N* 为情侣的总数。这里的并查集使用了路径压缩，但是没有使用按秩合并，最坏情况下的时间复杂度是 ![O(N\logN) ](./p__O_N_log_N__.png) ，平均情况下的时间复杂度依然是 ![O(N\alpha(N)) ](./p__O_N_alpha__N___.png) ，其中 ![\alpha ](./p__alpha_.png)  为阿克曼函数的反函数，![\alpha(N) ](./p__alpha__N__.png)  可以认为是一个很小的常数。
